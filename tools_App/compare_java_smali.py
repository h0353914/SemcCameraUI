...
def _method_bodies_equivalent(body1: list[str], body2: list[str]) -> bool:
    ...
    # 策略 10.5: 暫存器抄除序列比較 (register-erased sequence comparison)
    # 將所有暫存器名稱命名為 'R'、標籤命名為 ':L'，保留操作碼 + 欄位/方法引用 + 常數就做順序比較。
    # 安全性：兩個版本是相同源碼的編譯產物，數据流相同；
    #   暫存器名稱只是實現細節（dx 重用 p0、D8 加新 v0），不影響語意。
    # 解決主要失敗模式：純暫存器成名差異（p0 vs v0）導致 bijective mapping 失敗。
    def _make_erased(ins: list[str], pipeline_fn) -> list[str]:
        pipelined = pipeline_fn([x for x in ins if x.strip()])
        normalized = _normalize_instructions(pipelined)
        result = []
    ...
    def _no_branch_pipeline_s105(ins: list[str]) -> list[str]:
        r = _expand_stringbuilder_init(ins)
        r = _merge_adjacent_stringbuilder_appends(r)
        r = _strip_sdk_int_guards(r)
        r = _collapse_access_to_field(r)
    ...
    erased1_full = _make_erased(instrs1, _full_pipeline)
    erased2_full = _make_erased(instrs2, _full_pipeline)
    if erased1_full == erased2_full:
        return True

    erased1_nb = _make_erased(instrs1, _no_branch_pipeline_s105)
    erased2_nb = _make_erased(instrs2, _no_branch_pipeline_s105)
    if erased1_nb == erased2_nb:
        return True
...
def analyze_diff(
    ...
    for sig_j, sig_o in method_mapping.items():
        ...
        # 策略 S12e: 暫存器抄除序列比較（檔案層級）
        # 與方法態策略 10.5 相同，但先套用典範名映射解決 JADX 重命名差異
        def _erased_seq(ins: list[str]) -> list[str]:
            result = []
            for line in ins:
                s = line.strip()
                if not s or s.startswith(".") or re.match(r"^:[a-zA-Z_]\w*$", s):
        ...
        erased_j = _erased_seq(_normalize_instructions(en_j))
        erased_o = _erased_seq(_normalize_instructions(en_o))
        if erased_j == erased_o:
            if DiffKind.REGISTER_RENAME not in diff_kinds:
                diff_kinds.append(DiffKind.REGISTER_RENAME)
            continue
