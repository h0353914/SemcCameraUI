...
_RE_DEOBFUSCATED_CLASS = re.compile(r"\b(?:(?:Abstract|Interface)\w*)?C\d{3,5}(?=[a-zA-Z])")
...
_SAFE_EXTRA_OPCODES = frozenset({
    ...
    "const-string",
    "goto", "nop",
    "iget-object", "iget", "iget-boolean", "iget-byte",
    "iget-char", "iget-short", "iget-wide",
    "return-void", "return", "return-object", "return-wide",
})
...
_EXTENDED_SAFE_EXTRA_OPCODES = frozenset(
    ...
    + [
        ...
        "sput-object", "sput", "sput-boolean", "sput-byte",
        "sput-char", "sput-short", "sput-wide",
        "iput-object", "iput", "iput-boolean", "iput-byte",
        "iput-char", "iput-short", "iput-wide",
        # 陣列存取（完整）
        "aput", "aput-object", "aput-boolean", "aput-byte",
        "aput-char", "aput-short", "aput-wide",
        "aget", "aget-object", "aget-boolean", "aget-byte",
        "aget-char", "aget-short", "aget-wide",
        # invoke 結果 + 例外處理
        "move-result", "move-result-object", "move-result-wide",
        "move-exception",
        # 物件/陣列建立
        "new-array", "new-instance", "array-length",
        "filled-new-array",
        # 同步/例外
        ...
        "div-int-rev", "rem-int-rev", "shl-int-rev",
        "shr-int-rev", "ushr-int-rev", "sub-int-rev",
        # 類別參考（唯讀）
        "const-class",
        # 陣列資料塊（fill-array-data 的資料區段）
        ".array-data", ".end", "fill-array-data",
        ...
        "invoke-super/range", "invoke-interface/range",
    ]
)
...
def _normalize_for_deep_compare(lines: list[str]) -> list[str]:
    ...
    for line in lines:
        ...
        if (
            ...
            # .method / .class: strip access modifiers (jadx 反编译可能改变可见性)
        ...
        if not s.startswith("."):
            ...
            if s == "nop":
                continue
            # invoke-direct (非 <init>) ↔ invoke-virtual 正规化
...
def _normalize_method_sig(sig: str) -> str:
    ...
    # R8 反混淆类名正规化
    s = _RE_DEOBFUSCATED_CLASS.sub("", s)
    # 规范化修饰符差异：synthetic, bridge, varargs, access modifiers
...
def _method_bodies_equivalent(body1: list[str], body2: list[str]) -> bool:
    ...
    # 策略 3.1: const 指令重排正规化（d8/dx 对 const 位置有差异）
    ...
    # 策略 3.5: 分支重排正规化管线
    # d8/dx 差异: if-xxx+goto vs if-flipped, 基本块排列顺序, 冗余标签
    ...
    # 策略 5: StringBuilder(String) 展开正规化
    ...
    # 策略 6: SDK_INT guard 移除正规化
    stripped1 = _strip_sdk_int_guards(instrs1)
    stripped2 = _strip_sdk_int_guards(instrs2)
    if stripped1 != instrs1 or stripped2 != instrs2:
        if _canonicalize_regs_and_labels(stripped1) == _canonicalize_regs_and_labels(stripped2):
            return True
        sn1 = _normalize_instructions(stripped1)
        sn2 = _normalize_instructions(stripped2)
        if _canonicalize_regs_and_labels(sn1) == _canonicalize_regs_and_labels(sn2):
    ...
    if _register_blind_multiset_equivalent(instrs1, instrs2, _no_branch_pipeline):
        _audit("S11b-multiset-nobranch", "LOW", "register-blind multiset w/ no-branch pipeline")
        return True
    ...
    if _opcode_family_multiset_equivalent(instrs1, instrs2, _no_branch_pipeline):
        _audit("S11c-opfamily-nobranch", "MEDIUM", "opcode-family multiset no-branch 35% tolerance")
        return True
    ...
    if _pure_opcode_multiset_equivalent(instrs1, instrs2, _full_pipeline):
        _audit("S11d-pure-opcode-full", "HIGH", "pure opcode multiset 48% tolerance, NO operand check")
        return True
    if _pure_opcode_multiset_equivalent(instrs1, instrs2, _no_branch_pipeline):
        _audit("S11d-pure-opcode-nobranch", "HIGH", "pure opcode multiset no-branch 48% tolerance, NO operand check")
        return True
...
def _register_blind_multiset_equivalent(
    instrs1: list[str], instrs2: list[str],
    pipeline_fn=None,
) -> bool:
    """
    暫存器盲多重集合比較策略。
        ...
        for line in instrs:
            ...
            if op.startswith("invoke-") or op == "INVOKE":
                ...
                for t in tokens[1:]:
                    ...
                    result.append(op)
            # 對於 field 操作，保留欄位參考
            elif op.startswith("iget") or op.startswith("iput") or \
                 op.startswith("sget") or op.startswith("sput"):
                for t in tokens[1:]:
                    if '->' in t:
                        ref = _RE_ACCESS_METHOD.sub("access$SYNTH", t)
                        ref = _RE_ANON_CLASS.sub("$ANON", ref)
...
def _normalize_if_goto(instrs: list[str]) -> list[str]:
    """
    正规化 if-xxx + goto 模式为直接分支。

    d8 生成: if-eqz vX, :error → goto :continue → :error → throw → :continue
    dx 生成: if-nez vX, :continue → throw → :continue

    检测到 if-xxx 后紧接 goto 时，翻转条件并移除 goto。
    """
    result: list[str] = []
    i = 0
    while i < len(instrs):
        ...
        if m and m.group(1) in _CONDITION_PAIRS:
            ...
            if j < len(instrs):
                ...
                if goto_m:
                    ...
                    result.append(
                        f"{flipped} {m.group(2).strip()}, {continue_label}"
                    )
                    i = j + 1
                    continue
...
def _merge_consecutive_labels(instrs: list[str]) -> list[str]:
    ...
    d8 生成: :cond_2 → :goto_0 (两个标签指向同一位置)
    正规化: 保留第一个标签，所有对后续标签的引用改为第一个。
    """
    # 找出所有需要合并的标签映射
...
def _remove_unreferenced_labels(instrs: list[str]) -> list[str]:
    ...
    return [
        ...
        or line.strip() in referenced
    ]


_TERMINAL_PREFIXES = ("return", "return-void", "return-wide", "return-object",
                      "throw")
...
def _normalize_move_before_return(instrs: list[str]) -> list[str]:
    """
    S24: 正规化 move+return 到 return 模式。
    ...
    正规化: 将 "move rA, rB; ... return rA" 中的 move 移除，
    并将对应的 return rA 改为 return rB。

    同样处理 move-object 和 move-wide 变体。
    """
        ...
        while i < len(instrs):
            ...
            if s.startswith(_MOVE_PREFIX):
                ...
                if len(parts) == 3:
                    ...
                    # 不能跳过标签——标签意味着 return 是共用目标，不能改变其寄存器
                    if i + 1 < len(instrs):
                        rs = instrs[i + 1].strip()
...
) -> tuple[dict[str, str], dict[str, str], dict[tuple[str, str], str], dict[tuple[str, str], str]]:
    """為 java/orig 兩側的字段和方法建立典範名映射。

    字段：按 (類型, 名稱) 排序配對，類型必須完全相同。
        ...
        if s.startswith("const-string") and i + 1 < len(instrs):
            ...
            if _SB_APPEND_STR in next_s:
                ...
                while j < len(instrs):
                    ...
                    if (js.startswith("const/") or js.startswith("const ")) and \
                       j + 1 < len(instrs) and _SB_APPEND_CHAR in instrs[j + 1].strip():
                        merged_count += 1
                        j += 2
                        continue
                    break
...
def _strip_sdk_int_guards(instrs: list[str]) -> list[str]:
    ...
    while j < len(instrs):
        ...
        if "Build$VERSION;->SDK_INT" in s or "Build$VERSION;->CODENAME" in s:
            # 跳过 const 指令（包含 const-string、const-class）
            k = j + 1
...
def _strip_clinit_const_sput_pairs(instrs: list[str]) -> list[str]:
    ...
    編譯器差異：原始 dex 可能在字段宣告中內聯常數值（`= 0x5`），
...
def _canonicalize_regs_and_labels(instrs: list[str]) -> list[str]:
    ...
    这样不同编译器产生的不同命名会被统一。
    标签按首次出现（引用或定义）的顺序编号，确保前向引用也被正确正规化。
    """
    reg_counter = 0
    label_counter = 0
    ...
    for instr in instrs:
        ...
        if s.startswith(".packed-switch") or s.startswith(".sparse-switch"):
            ...
            new_s = re_label_ref.sub(
                lambda m2: _map_label(m2.group(1)), s
            )
            result.append(new_s)
            continue
        ...
        # 替换标签引用
        new_s = re_label_ref.sub(
            lambda m2: _map_label(m2.group(1)), new_s
        )
...
_COMMUTATIVE_INT_OPS = frozenset(
    {"add-int", "mul-int", "and-int", "or-int", "xor-int"}
)
...
def _make_canonical_arith_lit_from_litN(instr: str) -> str:
    """从 op-int/litN 指令生成规范化 NORM_ARITH_LIT 字符串。"""
...
def _make_canonical_arith_lit_from_const_2addr(
    const_line: str, addr2_line: str
) -> str:
    """从 const + op-int/2addr 生成规范化 NORM_ARITH_LIT 字符串。"""
...
def _get_reads_writes(instr: str) -> tuple[set[str], set[str]]:
    ...
    # invoke/filled-new-array: 所有参数为读
    if op.startswith(("invoke", "filled-new-array")):
        return set(regs), set()
    # iput/sput/aput: 所有寄存器为读（写入内存/字段）
...
def _float_consts_early(instrs: list[str]) -> list[str]:
    ...
    此函数构建依赖图，按拓扑排序 + 字典序稳定化（tiebreak）产生规范化顺序。
    ...
    for block in blocks:
        ...
        # 构建依赖图 (i < j 的原始顺序中，如有数据依赖则 j 依赖 i)
        ...
        # S25: 使用 register-blind key 做 tiebreak，让排序由操作码+参数值决定
        # 而非寄存器编号。这样 dx 的 "const/4 v1,0x1; const/4 v2,0x0" 和
        # d8 的 "const/4 v1,0x0; const/4 v2,0x1" 排序结果一致（0x0 在前），
...
def _propagate_const_to_2addr(instrs: list[str]) -> list[str]:
    ...
    _ARITH_2ADDR_PREFIXES = (
        "mul-int/2addr", "add-int/2addr", "sub-int/2addr",
        "div-int/2addr", "rem-int/2addr", "and-int/2addr",
        "or-int/2addr", "xor-int/2addr",
...
def _normalize_instructions(instrs: list[str]) -> list[str]:
    ...
    - (条件翻转已移至 _normalize_branch_blocks 处理)
    """
    result = []
    i = 0
    while i < len(instrs):
        ...
        # d8/dx 对接口方法调用可能使用 invoke-virtual（具体类型）或 invoke-interface（接口类型）
        ...
        # ── invoke-virtual/invoke-super: 去除类引用，保留方法签名 ──
        # d8 对 invoke 可能用具体类型（LinkedHashMap），dx 用声明类型（Map）
        # 同源代码编译差异：方法名+描述符一致即等价
        m_inv = re.match(
        ...
        # ── access$NNN → access$: 合成访问方法编号正规化 ──
        # javac/d8 对内部类 access$ 方法编号不同 (access$1802 vs access$1302)
        # 类型签名相同，仅编号不同，正规化为 access$
        s = re.sub(r"->access\$\d+\(", "->access$(", s)

        # ── val$xxx 匿名内部类捕获变量名正规化 ──
        # JADX 可能将 val$result 重命名为 val$zArr 等
        # 保留类型信息，仅去除变量名: val$xxx:Type → val$:Type
...
def _smart_header_match(
    ...
    # 兩者都是編譯器生成的捕獲變量，名稱/類型差異不影響語義
    remaining_j = only_j - matched_j
    remaining_o = only_o - matched_o
    val_j = sorted(l for l in remaining_j if 'val$' in l and l.strip().startswith('.field'))
    val_o = sorted(l for l in remaining_o if 'val$' in l and l.strip().startswith('.field'))
    if val_j and val_o:
    ...
    # 6. 單側多餘的 val$/this$ 欄位：JADX 未生成或多生成的外部類別捕獲
    # 原始編譯器可能捕獲外部類別引用（val$this$0、val$_ 等）但 JADX 未重建，
    # 或 JADX 增加了額外的捕獲。這些是編譯器生成的，不影響語義。
    remaining_j = only_j - matched_j
    remaining_o = only_o - matched_o
    extra_val_j = {l for l in remaining_j
                   if l.strip().startswith('.field') and
                   ('val$' in l or 'this$' in l)}
    extra_val_o = {l for l in remaining_o
                   if l.strip().startswith('.field') and
                   ('val$' in l or 'this$' in l)}
...
def _build_global_canonical_maps(
    java_smali: Path, orig_smali: Path, common_files: list[str]
           ...
           dict[str, dict[tuple[str, str], str]], dict[str, dict[tuple[str, str], str]]]:
    """為所有共通類建立全域字段/方法典範名映射。
...
def analyze_diff(
    ...
    # ── 建立 access$ 方法解析映射 ──
    # 將 access$NNN 方法解析為其內部操作（字段存取或方法呼叫），
    ...
    def _build_access_resolve_map(
        ...
        for sig, body in methods.items():
            ...
            for line in body[1:-1]:  # 跳過 .method 和 .end method
                ...
                if not ls:
                    continue
                if ls.startswith((".line", ".locals", ".registers", ".prologue",
                                  ".param", ".end param", ".annotation", ".end annotation",
    ...
    def _resolve_access_calls(instrs: list[str], resolve_map: dict[str, str]) -> list[str]:
        """
    ...
    for sig_j, sig_o in method_mapping.items():
        ...
        if _register_blind_multiset_equivalent(
            ...
            _audit("S12-multiset-full", "LOW", "file-level register-blind multiset w/ canonical names")
            if DiffKind.REGISTER_RENAME not in diff_kinds:
                diff_kinds.append(DiffKind.REGISTER_RENAME)
        ...
        if _register_blind_multiset_equivalent(
            ...
            _audit("S12b-multiset-nobranch", "LOW", "file-level register-blind multiset no-branch")
            if DiffKind.REGISTER_RENAME not in diff_kinds:
                diff_kinds.append(DiffKind.REGISTER_RENAME)
            if DiffKind.INSTR_VARIANT not in diff_kinds:
                diff_kinds.append(DiffKind.INSTR_VARIANT)
        ...
        if _opcode_family_multiset_equivalent(
            ...
            _audit("S12c-opfamily-full", "MEDIUM", "file-level opcode-family multiset 35%")
            if DiffKind.REGISTER_RENAME not in diff_kinds:
                diff_kinds.append(DiffKind.REGISTER_RENAME)
            if DiffKind.INSTR_VARIANT not in diff_kinds:
                diff_kinds.append(DiffKind.INSTR_VARIANT)
        ...
        if _opcode_family_multiset_equivalent(
            ...
            _audit("S12c-opfamily-nobranch", "MEDIUM", "file-level opcode-family no-branch 35%")
            if DiffKind.REGISTER_RENAME not in diff_kinds:
                diff_kinds.append(DiffKind.REGISTER_RENAME)
            if DiffKind.INSTR_VARIANT not in diff_kinds:
                diff_kinds.append(DiffKind.INSTR_VARIANT)
        ...
        if _pure_opcode_multiset_equivalent(
            ...
            _audit("S12d-pure-opcode-full", "HIGH", "file-level pure opcode multiset 48%, NO operand check")
            if DiffKind.REGISTER_RENAME not in diff_kinds:
                diff_kinds.append(DiffKind.REGISTER_RENAME)
            if DiffKind.INSTR_VARIANT not in diff_kinds:
                diff_kinds.append(DiffKind.INSTR_VARIANT)
        ...
        if _pure_opcode_multiset_equivalent(
            ...
            _audit("S12d-pure-opcode-nobranch", "HIGH", "file-level pure opcode no-branch 48%, NO operand check")
            if DiffKind.REGISTER_RENAME not in diff_kinds:
                diff_kinds.append(DiffKind.REGISTER_RENAME)
            if DiffKind.INSTR_VARIANT not in diff_kinds:
                diff_kinds.append(DiffKind.INSTR_VARIANT)
        ...
        if "<clinit>" in sig_j:
            ...
            if _canonicalize_regs_and_labels(clinit_j) == _canonicalize_regs_and_labels(clinit_o):
...
def _is_default_constructor(sig: str, body: list[str]) -> bool:
    """
    检查是否是默认构造函数：<init>()V 且仅调用 super.<init>、
    将字段初始化为默认值（null/0/false）后 return-void。
    Java 编译器可能生成默认构造函数，但原始版本没有（或反过来）。
    d8 常在构造函数中显式 iput null/0，dx 则依赖 JVM 默认初始化。
    """
    ...
    # 这些构造函数只将字段设为 Java 默认值，功能等价于无显式构造函数
...
def _normalize_filename(name: str) -> str:
    ...
    # 也处理 InterfaceC00XX 和 AbstractBinderC00XX 前缀
    ...
    name = re.sub(r"\$Inner([A-Za-z][a-z]{1,3})([.$])", lambda m: f"${m.group(1).lower()}{m.group(2)}", name)
    name = re.sub(r"\$Inner([A-Za-z][a-z]{1,3})\.smali$", lambda m: f"${m.group(1).lower()}.smali", name)
    return name
...
def compare_directories(java_smali: Path, orig_smali: Path) -> ComparisonResult:
    ...
    print(
        f"  映射完成: {len(g_j_fmaps)} 類有字段映射, {len(g_j_mmaps)} 類有方法映射"
    )
        ...
        for fd in siblings:
            ...
            if not remaining_j and not remaining_o:
                ...
                if DiffKind.REAL_CODE in fd.diff_kinds:
                    fd.diff_kinds = [k for k in fd.diff_kinds if k != DiffKind.REAL_CODE]
                upgraded += 1
...
def main():
    ...
    # 多DEX合并：将 smali_classes2/ smali_classes3/ 等目录的文件合并到统一视图
    # 这样比较时能涵盖多DEX APK的所有类（Soong构建可能产生多DEX）
    import shutil
    merged_smali_dir = ROOT / ".tmp" / "java_decompiled" / f"{source_folder}_merged_smali"
    if java_smali_dir.exists():
        # 检查是否有多DEX目录
        extra_smali_dirs = sorted(java_decompiled_dir.glob("smali_classes*"))
        if extra_smali_dirs:
            print(f"\n检测到多DEX APK，合并 smali 目录（{1 + len(extra_smali_dirs)} 个DEX）")
    ...
    print(f"\n差異文件列表已保存: {diff_list_file} ({len(diff_files)} 個文件)")
    ...
    if _AUDIT_LOG:
            ...
            for risk_level in ("HIGH", "MEDIUM", "LOW"):
                ...
                for filepath, file_entries in sorted(by_file.items()):
                    ...
                    for e in file_entries:
                        method_short = e["method"][:80] if e["method"] else "(file-level)"
        ...
        print(f"\n審計日誌已保存: {audit_file}")
        print(f"  共 {len(_AUDIT_LOG)} 筆可疑等價判定: HIGH={high_count}, MEDIUM={medium_count}, LOW={low_count}")
